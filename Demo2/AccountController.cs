...
namespace Driving_License.Controllers
{
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> GetAccountList(string keyword = "", string role = "", int page = 1)
        {
            ...
            var accounts = _context.Accounts.Include(x => x.User).Include(x => x.Teacher).Include(x => x.Admin).Include(x => x.Staff).Where(x => !x.Role.Equals("admin")).AsQueryable();
        ...
        public async Task<IActionResult> Edit([FromRoute] Guid accountid, [FromForm] IFormCollection formData)
        {
            try
            {
                ...
                Account old_account = await _context.Accounts.Include(x => x.User).Include(x => x.Teacher).Include(x => x.Admin).Include(x => x.Staff).SingleOrDefaultAsync(acc => acc.AccountId.Equals(accountid));
                if (old_account == null)
                {
                ...
                if(old_account.User != null)
                {
                    old_account.User.Email = email;
                }
                if(old_account.Teacher != null)
                {
                    old_account.Teacher.Email = email;
                }
                if(old_account.Staff != null)
                {
                    old_account.Staff.Email = email;
                }
                await _context.SaveChangesAsync();
        ...
        {
            // Add your logic to validate if the role is valid in your system
            // For example, check if the role exists in a predefined list of roles
            // Return true if valid, false otherwise
            // You might want to consider using an enum for roles for better validation
