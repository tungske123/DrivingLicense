...
async function submitQuiz() {
    ...
    if (result.isConfirmed) {
        window.location.href = `/Quiz/ViewQuizResult?uid=${UserId}&aid=${attempt.attemptId}&qid=${parseInt(quiz.quizId)}`;
    }
}
...
function renderQuestionButtons(questions) {
    ...
    const normalButtonClassName = `btn btn-outline-dark`;
    const activeButtonClassName = `btn btn-secondary`;
    const doneButtonClassName = `btn btn-success`;
    for (let i = 0; i < questions.length; ++i) {
        ...
        button.addEventListener('click', async () => {
            var qid = parseInt(button.getAttribute('qid'));
    ...
    let questionBtnList = document.querySelectorAll('.quiz-select-section button');
    questionBtnList.forEach(btn => {
        let qid = parseInt(btn.getAttribute('qid'));
        if (parseInt(qid) === parseInt(currentQuestionId)) {
            btn.className = activeButtonClassName;
        }
    });
}
