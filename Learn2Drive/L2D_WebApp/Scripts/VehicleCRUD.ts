...
function getVehicleFormattedPrice(price: number) {
    return price.toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g, ".").toString();
}

var currentVehicleID: string = ``;

function renderVehicleTable(vehicleList: VehicleCRUD[]) {
    let template = document.getElementById('VehicleCRUD-row-template') as HTMLTemplateElement;
...
async function fetchSingleVehicleData(vehicleId: string): Promise<VehicleCRUD> {
    ...
    try {
        ...
        const data = await response.json();
        const VehicleCRUD: VehicleCRUD = data;
        return VehicleCRUD;
    } catch (error) {
        console.error(`Error! ${error}`);
    }
...
async function loadVehicleToDetailsModal(vehicleId: string) {
    const VehicleCRUD: VehicleCRUD = await fetchSingleVehicleData(vehicleId);
